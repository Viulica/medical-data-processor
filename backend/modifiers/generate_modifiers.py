...
def apply_colonoscopy_correction(row, asa_code, insurances_df):
    ...
    Every other case keeps the AI-predicted code.
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            if has_peripheral_blocks:
                ...
                if peripheral_blocks_mode == "UNI":
                    # UNI mode: gating TEMPORARILY DISABLED — the Anesthesia Type
                    # gate was silently dropping legitimate secondary/post-op
                    # blocks whenever the PRIMARY anesthesia type wasn't
                    # General/Neuraxial (e.g. REGIONAL, MAC, or blank primaries
                    # that still had a documented post-op block). Force-generate
                    # and let the extraction's peripheral_blocks field decide.
                    # Original gate kept below (commented) so it can be restored.
