...
Respond with ONLY the JSON object, nothing else."""
    ...
    for attempt in range(max_retries):
        try:
            ...
            err_body = response_data.get("error") if isinstance(response_data, dict) else None
            if err_body and "choices" not in response_data:
                ...
                if transient:
                    if payload.get("service_tier") == "flex":
                        if flex_503_retries < 2:
                            flex_503_retries += 1
                            wait_time = 2 ** flex_503_retries
                            logger.warning(f"OpenRouter 200+error-body on flex tier (CPT, code={err_code}); flex retry {flex_503_retries}/2 in {wait_time}s")
                            time.sleep(wait_time)
                            continue
                        else:
                            logger.warning(f"OpenRouter 200+error-body on flex tier (CPT) after 2 retries; falling back to standard tier")
                            payload.pop("service_tier", None)
                            payload.pop("provider", None)
                            flex_disabled = True
                            continue
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(f"OpenRouter 200+error-body (CPT, code={err_code}); retry {attempt+1}/{max_retries} in {wait_time}s")
                        time.sleep(wait_time)
                        continue
                ...
                if transient:
                    if payload.get("service_tier") == "flex":
                        if flex_503_retries < 2:
                            flex_503_retries += 1
                            wait_time = 2 ** flex_503_retries
                            logger.warning(f"OpenRouter 200+error-body on flex tier (ICD, code={err_code}); flex retry {flex_503_retries}/2 in {wait_time}s")
                            time.sleep(wait_time)
                            continue
                        else:
                            logger.warning(f"OpenRouter 200+error-body on flex tier (ICD) after 2 retries; falling back to standard tier")
                            payload.pop("service_tier", None)
                            payload.pop("provider", None)
                            flex_disabled = True
                            continue
                    if attempt < max_retries - 1:
                        wait_time = min(2 ** attempt, 8)
                        logger.warning(f"OpenRouter 200+error-body (ICD, code={err_code}); retry {attempt+1}/{max_retries} in {wait_time}s")
                        time.sleep(wait_time)
                        continue
...
def _build_cpt_guidance_block(predicted_cpt):
    ...
    return (
        "\n\n"
        "================================================================================\n"
        "PREDICTED ANESTHESIA CPT FOR THIS CASE (supporting context only)\n"
        "================================================================================\n"
        f"{cpt_line}\n"
        "\n"
        "NOTE on the description above: it is the GENERIC anesthesia-CPT description pulled\n"
        "from a standard reference list (not specific to this patient). The CPT itself is just\n"
        "the anesthesia code for the broad procedure category — use it only as supporting\n"
        "context to narrow down the procedure type, NOT as the source of truth for the ICD.\n"
        "\n"
        "Your ICD codes must be chosen based on what is actually documented in this specific\n"
        "PDF (pre-op diagnosis, post-op diagnosis, indication, surgeon's note, etc.) — pick\n"
        "the ICD codes that medically justify the procedure performed on this patient.\n"
        "Do not let the generic CPT description override what the chart actually says.\n"
        "\n"
        "Apply any facility-specific CPT→ICD rules from the prompt above using this CPT.\n"
        "Sanity-check: ensure your ICD1 is clinically consistent with the procedure category\n"
